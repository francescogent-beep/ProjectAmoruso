...
const EntryProgram: React.FC = () => {
  ...
  const optimizedImage = entryProgram.image.replace(/\.(jpe?g|png)$/i, 'l.jpg');
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="bg-brand-gray border border-white/5 p-8 md:p-16 flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="md:w-2/3">
            ...
            <h2 className="font-display text-4xl md:text-5xl font-black uppercase italic mb-6 text-white">
              {entryProgram.title} <br />
              <span className="gold-gradient">{entryProgram.accent}</span>
            </h2>
            ...
            <ul className="grid sm:grid-cols-2 gap-4 mb-10 text-sm text-white font-medium">
              {entryProgram.features.map((feature, idx) => (
                <li key={idx} className="flex items-center space-x-3">
                  <span className="w-1.5 h-1.5 gold-metallic-bg"></span>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-8">
              <a 
                ...
                className="w-full sm:w-auto gold-metallic-bg px-12 py-5 font-black uppercase tracking-widest text-base hover:scale-105 transition-all text-center inline-block shadow-xl"
              >
                {entryProgram.cta}
              </a>
